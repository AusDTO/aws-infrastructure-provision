...
function getvars() {
  ...
  BOSH_RDS_HOST="$(terraform output -state=terraform/environment/terraform.tfstate bosh_rds_host)"
  BOSH_RDS_USERNAME="$(terraform output -state=terraform/environment/terraform.tfstate bosh_rds_username)"
  BOSH_RDS_PASSWORD="$(terraform output -state=terraform/environment/terraform.tfstate bosh_rds_password)"
}
